...
/* 
 * 64 bit magic FNV-0 prime 
 */
#if defined(HAVE_64BIT_LONG_LONG)
#define FNV_64_PRIME ((Fnv64_t)0x100000001b3ULL)	/* 64 bit FNV-0 prime */
#else
 ...
 */
Fnv64_t
fnv0_64_buf(void *buf, size_t len, Fnv64_t hval)
{
#if defined(HAVE_64BIT_LONG_LONG)
    ...
    /*
     * FNV-0 hash each octet of the buffer
     */
    while (bp < be) {
 ...
 */
Fnv64_t
fnv0_64_str(char *str, Fnv64_t hval)
{
#if defined(HAVE_64BIT_LONG_LONG)

    /*
     * FNV-0 hash each octet of the string
     */
    while (*str) {
